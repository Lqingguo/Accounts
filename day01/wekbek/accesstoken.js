...
const {writeFileAsync, readFileAsync}=require('../utils/toop')
class Wechat {
  /**
   * 获取access_token
   ...
   * @returns {Promise.<*>}
   */
    async getTag(tagid,next_openid){
      try {
        const {access_token} = await this.fetchAccessToken();
   ...
   async uploadMaterial (type,material,body){
     try {
       ...
       if(type === 'news'){
         ...
         //以form表单上传
         options.formData={
           media:createReadStream(meterial)
         }
       }else {
         url=`${api.upload.uploadOthers}access_tolen=${access_token}`
         //以form表单上传
         options.formData={
           media:creatReadStream(material)
         };
         if(type === 'video'){
           options.body = body;
         }
       ...
       options.url=url;
        ...
        return 'unploadMaterial方法出了问题'+ e;
     }
   }
}
//测试微信接口信息
(async () => {
  ...
  result = await w.creatMenu(require('./menue'));
